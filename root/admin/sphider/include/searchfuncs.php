...
function get_search_results($query, $start, $category, $searchtype, $results, $domain) {
	...
	if ($rows>0) {
		...
		while ($i < $num_of_results && $i < $results_per_page) {
            ...
            // Save the fulltext before formatting for Subpage Search
            $org_fulltxt = $fulltxt;
            ...
            // Add SubPage Parameters and append to the Result link URL
            $param = getlinkparameters($url, $org_fulltxt);
            if($param != NULL) {
                $url = $url . $param;
            }
	...
	// Functions defined for Subpage search operations
	function getlinkparameters($linkurl, $fulltxt) {
    
		global $mysql_table_prefix;
		$param = null;
		
		if(strpos($linkurl, 'slideplayer') != false) {  // if this is TRUE
			$fulltxt = returnwords($fulltxt, 5);
		}
		else if(strpos($linkurl, 'presentations') != false) {  // if this is TRUE
			$fulltxt = returnwords($fulltxt, 4);
		}
		else {
			$fulltxt = returnwords($fulltxt, 3);
		}
    
		$query = "select subpage_id FROM ".$mysql_table_prefix."link_subpages where subpagetxt like 
			'%$fulltxt%' and link_id=( select link_id from ".$mysql_table_prefix."links where 
				url='$linkurl')";
		...
		if(strpos($linkurl, 'slideplayer') != false) {  // if this is TRUE
			...
			if($pos != false) {
				$slideName = substr($linkurl,$pos + 1);
				$param = "#{wsApp=" . $slideName . ".pptx;wsSlide=" . $subpage_id . "}";
			}
		}
		else if(strpos($linkurl, 'aboutme') != false) {  // if this is TRUE
		
			if(strpos($linkurl, '?') == false) {
				$param = "?subpage=" . ($subpage_id+1);
			}
			else {
				$param = "&subpage=" . ($subpage_id+1);
			}
	...
	/*  Returns the first $wordsreturned out of $string.  If string
		contains fewer words than $wordsreturned, the entire string is returned. */

	function returnwords($string, $wordsreturned) {
    
		// remove all non alphanumeric characters
		$string = preg_replace("/&[a-z]{1,6};/", " ", $string);
