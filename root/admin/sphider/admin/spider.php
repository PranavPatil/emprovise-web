...
********************************************/
	...
	include "../../../lib/sphider/sphider_custom.php";
	...
	function index_url($url, $level, $site_id, $md5sum, $domain, $indexdate, $sessid, $can_leave_domain, $reindex) {
		...
		if ($url_status['state'] == 'ok') {
            ...
            // Get Data from the File for Subpages.
            $fileRawData = $file;
				...
				if ($OKtoIndex == 1) {
					...
					if (is_array($wordarray) && count($wordarray) > $min_words_per_page) {
						if ($md5sum == '') {
							...
							// Start of Subpage Condition
                            $subpageKey = is_url_subpage_enabled($url);

                            if($subpageKey != NULL) {
                                $subarray = getsubpage_array($fileRawData, $subpageKey);
                                $subpagecount = 0;
                                foreach ($subarray as $subtext) {
                                    if(strlen(trim($subtext)) > 0) {
                                       addsubpage($link_id, $subpagecount, $subtext, "ADD");
                                    }
                                    $subpagecount++;
                                }
                            }
							// End of Subpage Condition
                            ...
                            if($subpageKey != NULL) {
                                ...
                                foreach ($subarray as $subtext) {
                                    if(strlen(trim($subtext)) > 0) {
										addsubpage($link_id, $subpagecount, $subtext, "UPDATE");
                                    }
                                    $subpagecount++;
                                }
                            }
	...
	function index_site($url, $reindex, $maxlevel, $soption, $url_inc, $url_not_inc, $can_leave_domain) {
		global $mysql_table_prefix, $command_line, $mainurl,  $tmp_urls, $domain_arr, $all_keywords,
			   $audio_dir, $video_dir; // Refer to the directory paths for audio and video playlists
		...
		// Scan all Audio Tacklists
		$files = glob($audio_dir . "*.xml");
		foreach($files as $file) {
			print("<br /> Scanning Audio Playlist: " . $file);
			scanplaylist($file);
		}
		
		// Scan all Video Tacklists
		$files = glob($video_dir . "*.xml"); 
		foreach($files as $file) {
			print("<br /> Scanning Video Playlist: " . $file);
			scanplaylist($file);
		}
		echo "<br /><br />";
