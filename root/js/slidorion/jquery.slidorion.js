	...
	$.fn.extend({
		slidorion: function(options) {
			...
			return this.each(function() {
				...
				sliderCount = $('#newsslider > div', obj).size();
				...
				if(sliderCount==accordionCount){
					...
					$('#newsslider > div:eq('+(current-1)+')', obj).css('z-index',zPos);
					zPos++;
					
					if(effect != "fade" || effect != "none"){
						$('#newsslider > div', obj).css({'top':'0','left':'-600px'});
						$('#newsslider > div:eq('+(current-1)+')', obj).css({'top':'0','left':'0'});
					}
				...
				function animation(current, section, effect, speed, easingOption){
					if(!active && loaded){
						...
						$current = $('#newsslider > div:eq('+(current-1)+')', obj);
						$new = $('#newsslider > div:eq('+(section-1)+')', obj);
				...
				function resetLayers() {
					for(var i=sliderCount;i>0;i--){
						$('#newsslider > div:eq('+(i-1)+')', obj).css('z-index',zPos);
						zPos++;
					}
				}
